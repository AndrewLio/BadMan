...
void CommandMgr::execute(char* message) {
	...
	Game.getGuiData()->displayClientMessageF("[%sBadMan%s] %sCommand '%s' could not be found!", GOLD, WHITE, RED, cmd.c_str());
	...
	for (auto it = this->commandList.begin(); it != this->commandList.end(); ++it) {
		...
		if (!cmd.empty() && commandCopy.find(cmd) != std::string::npos)                                         // if it has the name in it but not completed
