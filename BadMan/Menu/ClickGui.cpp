...
void ClickGui::renderCategory(Category category) {
		...
		for (auto& mod : moduleList) {
			...
			if (allowRender) {
				if (!ourWindow->isInAnimation && !isDragging && rectPos.contains(&mousePos)) {  // Is the Mouse hovering above us?
					DrawUtils::fillRectangle(rectPos, MC_Color(255, 30, 30, 100), backgroundAlpha);  // 鼠标预选
					...
					DrawUtils::fillRectangle(rectPos, mod->isEnabled() ? MC_Color(255, 0, 0,200) : ClientColors::clickGuiModuleColor, backgroundAlpha);  // 子项
	...
	const float yOffset2 = ourWindow->pos.y;
	{
		...
		// Draw category component
		{
			...
			DrawUtils::fillRectangle(Vec4(rectPos.x, rectPos.w - 1, rectPos.z, rectPos.w), MC_Color(255, 0, 0, 255), 1 - ourWindow->animation);
