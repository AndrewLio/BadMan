...
void Fly::onEnable() {
	...
	case 5:
		if (Game.getLocalPlayer() != nullptr)
			Game.getLocalPlayer()->setPos((*Game.getLocalPlayer()->getPosition()).add(Vec3(0, 1, 0)));
		break;
	}
}
...
void Fly::onMove(MoveInputHandler *input) {
	...
	case 6: {
		...
		if (gameTick > 6) {
			if (!localPlayer->getMovementProxy()->isOnGround()) {
				Vec3 pos;
				pos.x = localPlayer->getPosition()->x;
				pos.y = localPlayer->getPosition()->y - 0.025f;
				pos.z = localPlayer->getPosition()->z;
