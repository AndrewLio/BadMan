...
void Breaker::onTick(GameMode* gm) {
	Vec3* pos = gm->player->getPosition();
	for (int x = (int)pos->x - range; x < pos->x + range; x++) {
		for (int z = (int)pos->z - range; z < pos->z + range; z++) {
			for (int y = (int)pos->y - range; y < pos->y + range; y++) {
				...
				if (destroy) {
					gm->survivalDestroyBlockHack(blockPos, 0, destroy, false);
	...
	if (treasures) {
		Game.forEachEntity([](Entity* ent, bool b) {
			...
			if (name.find("Treasure") != std::string::npos && Game.getLocalPlayer()->getPosition()->dist(*ent->getPosition()) <= 5) {
				Game.getLocalPlayer()->swingArm();
