...
Scaffold::Scaffold() : IModule(VK_NUMPAD1, Category::WORLD, "Automatically build blocks beneath you") {
	...
	registerFloatSetting("TowerSpeed", &towerspeed, towerspeed, 0.1, 1);
...
Vec3 Scaffold::getBlockBelow(Player* player, float yOffset) {
	...
	blockBelow.y -= player->aabb->size.y + yOffset;
	return blockBelow;
}

void Scaffold::adjustYCoordinate(Vec3& blockBelow, const Vec3& blockBelowReal) {
	blockBelow.y = blockBelowY.y;
	if (blockBelowReal.y < blockBelowY.y) {
		blockBelowY.y = blockBelowReal.y;
	}
}

void Scaffold::extendBlock(Player* player, const Vec3& velocity, Vec3& blockBelow) {
	int extenduo = RandomFloat(0, extend);
	blockBelow.x += velocity.x * extenduo;
	blockBelow.z += velocity.z * extenduo;
}
...
void Scaffold::onMove(MoveInputHandler* input) {
	...
	float speed = player->location->velocity.magnitudexz();
	GameSettingsInput* kinput = Game.getClientInstance()->getGameSettingsInput();
	if (GameData::isKeyDown(*kinput->spaceBarKey) && foundCandidate && speed < 0.16f) {
		player->location->velocity.y = towerspeed;
		Vec3 blockBelow = player->getRenderPositionComponent()->renderPos;  // Block 1 block below the player
		blockBelow.y -= player->aabb->size.y;
		blockBelow.y -= 0.5f;
		blockBelowY = blockBelow;
		nowtower = true;
	} else {
...
void Scaffold::onSendPacket(Packet* packet) {
	...
	if (hive || rotations) {
		...
		if (packet->isInstanceOf<C_MovePlayerPacket>()) {
			if (speed > 0.05f || nowtower) {
				auto* movePacket = reinterpret_cast<C_MovePlayerPacket*>(packet);
				Vec2 angle = Game.getLocalPlayer()->getPosition()->CalcAngle(blockBelow);
				movePacket->pitch = 83;
...
void Scaffold::onPlayerTick(Player* player) {
	...
	if (hive || rotations) {
		...
		if (speed > 0.05f || nowtower) {
			Vec2 angle = Game.getLocalPlayer()->getPosition()->CalcAngle(blockBelow);
			player->getActorRotationComponent()->pitch = angle.x;
