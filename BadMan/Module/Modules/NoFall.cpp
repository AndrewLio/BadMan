...
bool NoFall::isOverVoid() {
	for (float posY = Game.getLocalPlayer()->getPosition()->y; posY > 0.0f; --posY) {
		if (!(Game.getLocalPlayer()->getRegion()->getBlock(Vec3(Game.getLocalPlayer()->getPosition()->x, posY, Game.getLocalPlayer()->getPosition()->z))->blockLegacy->blockId == 0)) {
			return false;
		}
	}
	return true;
	...
	if (localPlayer != nullptr) {
		if (localPlayer->getMovementProxy()->getFallDistance() > 2.f) {
			...
			case 2: {
				...
				localPlayer->setPos((*localPlayer->getPosition()).add(0, (float)0.2, 0.f));
				break;
			}
			case 3: {
				PlayerActionPacket actionPacket;
