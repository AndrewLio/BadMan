...
bool CfindEntity(Entity* curEnt, bool isRegularEntity) {
	...
	Vec3 Eyepos = curEnt->getRenderPositionComponent()->renderPos;
	float dist = (curEnt->getHumanPos()).dist(Game.getLocalPlayer()->getHumanPos());
...
void CrystalAura::DestroyC(Entity* ent, int range) {
	if (Game.getLocalPlayer()->getHumanPos().dist(ent->getHumanPos()) < range && !dEnhanced) {
		Game.getGameMode()->attack(ent);
		Game.getLocalPlayer()->swingArm();
	} else if (dEnhanced) {
		for (auto& i : targetList)
			if (ent->getHumanPos().dist(i->getHumanPos()) < range) {
				Game.getGameMode()->attack(ent);
				Game.getLocalPlayer()->swingArm();
				return;
			}
