...
void Teleport::onTick(GameMode* gm) {
	...
	if (shouldTP && GameData::isKeyDown(*Game.getClientInstance()->getGameSettingsInput()->sneakKey)) {
		tpPos.y += (gm->player->getPosition()->y - gm->player->getAABB()->lower.y) + 1;  // eye height + 1
		if (bypass) {
			Game.getLocalPlayer()->lerpTo(tpPos, Vec2(1, 1), (int)std::fmax((int)(gm->player->getPosition()->dist(tpPos) * 0.1), 1));
		} else {
