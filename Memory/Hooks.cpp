...
void Hooks::Init() {
	...
	//	g_Hooks.LevelRendererPlayer__renderNameTagsHook = std::make_unique<FuncHook>(renderNameTags, Hooks::LevelRendererPlayer__renderNameTags);
	   ...
	   void* mob__getYBodyRotation = reinterpret_cast<void*>(FindSignature("48 83 EC 48 48 8B 41 08 48 8B D1 0F 29 74 24 ? 0F 28 F2 0F 29 7C 24 ? 0F 28 F9 48 8B 08 8B 42 10 48 8D 54 24 ? 89 44 24 58 E8 ? ? ? ? 48 85 C0 74 18"));
		g_Hooks.Mob__getYBodyRotationHook = std::make_unique<FuncHook>(mob__getYBodyRotation, Hooks::Mob__getYBodyRotations);
		...
		void* mob__swingduration = reinterpret_cast<void*>(FindSignature("48 89 5C 24 ? 57 48 83 EC 20 48 8B 15"));
		g_Hooks.Mob__getCurrentSwingDuraption = std::make_unique<FuncHook>(mob__swingduration, Hooks::Mob__getSwingDuration);
		...
		static auto bobViewHookF = [](__int64 _this, glm::mat4& matrix, float lerpT){
			...
			float degrees = fmodf(p->getPosition()->lerp(p->getPosition(), lerpT).x, 5) - 2.5f;
			degrees *= 180 / 2.5f;
...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (*message == cmdMgr->prefix) {
		...
		std::call_once(flag, [] {
			Game.getClientInstance()->getGuiData()->displayClientMessageF("%sYour BadMan prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
	...
	DrawUtils::setCtx(renderCtx, dat);
	{
...

		{
			...
			if (strcmp(screenName.c_str(), "start_screen") == 0) {
				...
				DrawUtils::drawImage("textures/BadMan/banner", imgPos, imgSize, Vec2(0, 0));*/

				// Draw BIG epic BadMan watermark
				{
					std::string text = "Badman";
...

				{
					...
					if (shouldRenderWatermark) {
						...
						DrawUtils::drawText(Vec2(rect.x + borderPadding, rect.y), &name, MC_Color(255, 0, 0, 200), nameTextSize);
						DrawUtils::drawText(Vec2(rect.x + borderPadding + nameLength, rect.w - 7), &version, MC_Color(255, 0, 0, 200), versionTextSize);
					}
				}
			}
		}
...
int Hooks::Mob__getSwingDuration(void* a1) {
	static auto oGetSwingDuration = g_Hooks.Mob__getCurrentSwingDuraption->GetFastcall<int, void*>();
...
void Hooks::chatLogHookFunc(__int64 a1, TextHolder* msg, uint32_t a2) {
	...
	if (message.getText() != nullptr && player != nullptr && Game.isInGame()) {
	} 
...
void Hooks::LocalPlayer__updateFromCamera(__int64 a1, Camera* camera, __int64* a3, Entity* a4) {
	...
	auto zoom = moduleMgr->getModule<Zoom>();
	if (zoom->isEnabled())
	camera->fieldOfView = zoom->strength;
...
void Hooks::Mob__getYBodyRotations(Entity* _this, float rotPrev, float rot) {  // setybodyrotations
	auto func = g_Hooks.Mob__getYBodyRotationHook->GetFastcall<void, Entity*, float, float>();
	static auto scaffold = moduleMgr->getModule<Scaffold>();
	if (_this == Game.getLocalPlayer()) {
		if(scaffold->isEnabled()) rotPrev = scaffold->bodyyaw;
	}
	func(_this, rotPrev, rot);
}
