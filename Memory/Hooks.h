...
//	static bool ReturnTrue(__int64 _this);
	...
	static void Mob__getYBodyRotations(Entity* _this, float rotPrev, float rot);
	static int Mob__getSwingDuration(void* a1);
	...
	std::unique_ptr<FuncHook> GameMode_continueDestroyBlockHook;
	...
	std::unique_ptr<FuncHook> Mob__getCurrentSwingDuraption;
	std::unique_ptr<FuncHook> Mob__getYBodyRotationHook;
